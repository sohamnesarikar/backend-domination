...
📘 _Matches any document where at least one comment was made by `user1`._
...
🧠 _Ensures that both conditions (`user` and `text`) are matched in the same comment object._
...
💥 _Checks if all values in the `$all` array are present inside `likes` array._
...
📈 _Simple query to access a nested numeric field._
...
```js
db.blogs.find({}, { title: 1, "comments.user": 1, _id: 0 });
```

📤 _Projection to selectively return only needed fields._
...
🎯 _Checks if `comments.user` array contains both values (even across multiple comment objects)._
...
📊 _Get how many posts went “viral.”_
...
👩‍💻 _Combines top-level and nested conditions._
...
🔎 _Projection along with filtering nested values._
